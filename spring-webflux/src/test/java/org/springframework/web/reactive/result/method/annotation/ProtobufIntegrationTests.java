...
public class ProtobufIntegrationTests extends AbstractRequestMappingIntegrationTests {
	...
	public void value() {
		...
		StepVerifier.create(result)
				.expectNext(ProtobufController.TEST_MSG)
				.thenCancel()
				.verify();
	}
	...
	static class ProtobufController {
		...
		@GetMapping("/messages")
		Flux<Msg> messages() {
			return interval(Duration.ofMillis(100), 50).map(l -> Msg.newBuilder().setFoo("Foo").setBlah(SecondMsg.newBuilder().setBlah(l.intValue()).build()).build());
		}
