...
public class Jaxb2XmlDecoder extends AbstractDecoder<Object> {
	 ...
	 * This is either the size the entire input when decoding as a whole, or when
	 * using async parsing with Aalto XML, it is size one top-level XML tree.
	 * When the limit is exceeded, {@link DataBufferLimitException} is raised.
	 * <p>By default this is set to 256K.
	 * @param maxInMemorySize the max number of bytes to buffer, or -1 for unlimited
	 * @since 5.1.11
	 */
	public void setMaxInMemorySize(int maxInMemorySize) {
		this.maxInMemorySize = maxInMemorySize;
		this.xmlEventDecoder.setMaxInMemorySize(maxInMemorySize);
	}
